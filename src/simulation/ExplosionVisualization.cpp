...
void ExplosionVisualization::drawDensities(Camera *camera, ExplosionSimulation *simulation) {
	if(config::drawDomain) {
		drawDomain();
	}
	Point cameraPosition = Point(camera->positionX, camera->positionY, camera->positionZ);
	Vector cameraDirection = Vector(camera->directionX, camera->directionY, camera->directionZ);

	int closest = 0, farest = 0;
	float closestDist = cameraPosition.distance(corners[0]);
	float farestDist = closestDist;
	...
	Vector planeNormal = Vector(-cameraDirection.x, -cameraDirection.y, -cameraDirection.z);
	Plane plane = Plane(planeNormal, corners[closest]);

	Point *intersections = new Point[cornersCount];
	for(int i = 0; i < cornersCount; ++i) {
		intersections[i] = plane.intersection(cameraPosition, Vector(cameraPosition, corners[i]));
	}

	Point upperPoint = Point(corners[closest].x, corners[closest].y + 10.0f, corners[closest].z);
	Point upperProjection = plane.intersection(upperPoint, cameraDirection);
	Vector up = Vector(corners[closest], upperProjection);
	up.normalize();
    ...
    for(int k = 1; k <= N; ++k) {
    	for(int j = 1; j <= N; ++j) {
    		for(int i = 1; i <= N; ++i) {
    			...
    			Point densityPoint = Point(elementSize*i + move.x, elementSize*j + move.y, -elementSize*k + move.z);
    			Vector toCamera = Vector(densityPoint, cameraPosition);
