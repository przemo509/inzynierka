...
void ExplosionSimulation::addSources() {
    ...
    if (currentFrame < config::mainSourceStartFrame) {
        return;
    }
        ...
        for (int j = 0; j < height; ++j) {
            for (int i = centerX - radius; i < centerX + radius; ++i) {
                ...
                if (currentFrame < config::mainSourceStartPhase2Frame) {
                    vy[i][j][k] += dt * config::mainSourcePhase1VY;
                } else if (currentFrame < config::mainSourceStartPhase3Frame) {
                    vy[i][j][k] += dt * config::mainSourcePhase2VY;
                } else {
                    vy[i][j][k] += dt * config::mainSourcePhase3VY;
                }
            }
        }
    }
...
void ExplosionSimulation::addForces() {
    ...
    for (int k = 0; k < size; ++k) {
        for (int j = 0; j < size; ++j) {
            for (int i = 0; i < size; ++i) {
                vy[i][j][k] += dt * (-1.0 * config::gravityFactor + dens[i][j][k] * config::thermalBuoyancyFactor);
                if (i < 5) {
...
void ExplosionSimulation::diffuse(BoundDirection dir, float factor, vect3f x, vect3f x0) {
    ...
    for (int step = 0; step < relaxationSteps; ++step) {
        for (int k = 1; k <= N; ++k) {
            for (int j = 1; j <= N; j++) {
                for (int i = 1; i <= N; i++) {
                    x[i][j][k] = (x0[i][j][k] + a * (x[i - 1][j][k] + x[i + 1][j][k] + x[i][j - 1][k] + x[i][j + 1][k] + x[i][j][k - 1] + x[i][j][k + 1]))
                            / precomputedDivider;
                }
            }
        }
...
void ExplosionSimulation::project(vect3f u, vect3f v, vect3f w, vect3f p, vect3f div) {
    for (int k = 1; k <= N; ++k) {
        for (int j = 1; j <= N; ++j) {
            for (int i = 1; i <= N; ++i) {
                div[i][j][k] = -(u[i + 1][j][k] - u[i - 1][j][k] + v[i][j + 1][k] - v[i][j - 1][k] + w[i][j][k + 1] - w[i][j][k - 1]) / 3.0 / N;
                p[i][j][k] = 0.0;
            }
        }
    }
    ...
    for (int step = 0; step < relaxationSteps; ++step) {
        for (int k = 1; k <= N; ++k) {
            for (int j = 1; j <= N; ++j) {
                for (int i = 1; i <= N; ++i) {
                    p[i][j][k] = (div[i][j][k] + p[i - 1][j][k] + p[i + 1][j][k] + p[i][j - 1][k] + p[i][j + 1][k] + p[i][j][k - 1] + p[i][j][k + 1]) / 6.0;
                }
            }
        }
        setBoundaries(NO_DIR, p);
