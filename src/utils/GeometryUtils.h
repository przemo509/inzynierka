...
class Vector {
	public:
		...
		Point translate(const Point &p, float factor) const {
...
class Plane {
	public:
		...
		Point intersection(const Point &start, const Vector &dir) {
			float f = dir.x*a + dir.y*b + dir.z*c; // to byłoby zerem gdyby dir był równoległy do płaszczyzny, ale u nas nie będzie
